...
const automationId = route.params.id as string;

useHead({
  title: "Edit Automation - CattyShack Automation",
  meta: [
    {
      name: "description",
      content: "Edit your automation rule",
    },
  ],
});

// Fetch automation data
const { data: automationResponse, status: automationStatus } = await useAsyncData(
  "automation",
  () => $fetch<{ message: string; data: HueAutomation }>(`/api/automation/hue/${automationId}`),
);
...
const isLoadingAutomation = computed(() => automationStatus.value === "pending");

// Initialize form state from loaded automation
...
// Watch for automation data and populate form
watch(
  () => automationResponse.value?.data,
  (automation) => {
    if (automation) {
      try {
...
async function onSubmit(event: FormSubmitEvent<AutomationCreate>) {
  try {
    await $fetch(`/api/automation/hue/${automationId}`, {
      method: "PUT",
      body: event.data,
    });
    ...
    toast.add({
      title: "Success",
      description: `Automation "${event.data.name}" updated successfully!`,
      color: "success",
    });

    // Navigate back to manage page
    router.push("/automations/manage");
  }
  catch (error: unknown) {
...
<template>
  <UMain>
    <UContainer class="py-8">
      ...
      <!-- Loading State -->
      <div v-if="isLoadingAutomation" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
